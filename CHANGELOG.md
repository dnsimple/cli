...
- Table output puts the header row in bold when the standard output stream is a terminal. `--no-color` and the `NO_COLOR` environment variable turn the bold off, and redirected output stays plain.
